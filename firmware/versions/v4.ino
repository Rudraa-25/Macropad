...
class ServerCallbacks : public NimBLEServerCallbacks {
  ...
  void onAuthenticationComplete(NimBLEConnInfo& ci) override {
    if(ci.isEncrypted()){
      ...
      Serial.println("[BLE] Auth failed — clearing bonds and disconnecting");
      // Clear ALL stored bonds so next attempt starts fresh
...
void setup(){
  ...
  // ---- WiFi AP (must come before BLE init on ESP32-C3) ----
  // Disable any previous WiFi state cleanly
  WiFi.disconnect(true);
  WiFi.mode(WIFI_OFF);
  delay(100);

  WiFi.mode(WIFI_AP);
  delay(200); // ESP32-C3 needs this gap before softAP()

  // Channel 6, not hidden, max 4 clients
  bool apOk = WiFi.softAP(WIFI_SSID, WIFI_PASS, 6, 0, 4);
  delay(500); // Let the AP fully initialise before reading IP

  IPAddress apIP = WiFi.softAPIP();
  if(apOk && apIP != IPAddress(0,0,0,0)){
    Serial.printf("[WiFi] AP OK  SSID: %s  IP: %s\n", WIFI_SSID, apIP.toString().c_str());
    updateOLED("WiFi AP Ready", apIP.toString().c_str(), "Connect to config");
  } else {
    Serial.println("[WiFi] AP FAILED — retrying once");
    delay(500);
    WiFi.softAP(WIFI_SSID, WIFI_PASS, 6, 0, 4);
    delay(500);
    apIP = WiFi.softAPIP();
    Serial.printf("[WiFi] AP retry  IP: %s\n", apIP.toString().c_str());
    updateOLED("WiFi AP Retry", apIP.toString().c_str(), "...");
  }
  delay(500);
  ...
  // ---- BLE (after WiFi is stable) ----
  // Clear any stale bond data from previous sessions on boot
  NimBLEDevice::init(DEVICE_NAME);
  NimBLEDevice::deleteAllBonds(); // wipe stale keys — forces clean pair every boot
  NimBLEDevice::setPower(ESP_PWR_LVL_P9);
  // Just Works pairing — no PIN, no confirm needed
  NimBLEDevice::setSecurityAuth(BLE_SM_PAIR_AUTHREQ_BOND); // bond only, no MITM
