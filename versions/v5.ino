// ============================================================
//  ESP32-C3 BLE Macropad — Firmware v5
//  Fixes in this version:
//    1. WiFi AP uses dedicated mode: hold K0 at boot to enter
//       config mode (BLE off, WiFi on). Normal boot = BLE only.
//       This solves the ESP32-C3 radio conflict completely.
//    2. Keys only send when bleState == CONNECTED (not PAIRING).
//       Pressing keys during pairing caused 0x213 timeout.
//    3. deleteAllBonds() removed from boot. Only clears on
//       auth failure. Bonded devices reconnect instantly.
//    4. OLED shows "Hold K0 = Config" so you know how to
//       enter WiFi mode.
// ============================================================
//
//  HOW TO USE:
//    Normal boot  → BLE keyboard mode (WiFi off)
//    Hold K0 at power-on → Config mode (WiFi AP on, BLE off)
//      Connect to "MacropadConfig" / "macropad123"
...
<div class="save-bar" id="saveBar">
  ...
  <button class="btn btn-primary" onclick="saveChanges()">Save to Flash 💾</button>
</div>
...
function saveChanges(){
  ...
  .then(r=>r.json()).then(d=>{if(d.ok){showToast('Saved ✓');setDirty(false);}else showToast('Save failed ✗');})
...
void setup() {
  Serial.begin(115200);
  delay(500);
  Serial.println("[BOOT] ESP32-C3 Macropad v5");
